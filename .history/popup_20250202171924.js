...
chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
  ...
  currentUrlDisplay.textContent = url.href;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  console.log("ddd addEventListener")
...
function fillAccount(index) {
    ...
    chrome.storage.local.get([currentUrl], function(result) {
        ...
        chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
            ...
            console.log("fillAccount jin",tabs[0].id)
...
function fillFormFields(username, password) {
    console.log("fillFormFields", username, password);
    // 用户名输入框选择器
    ...
    usernameSelectors.forEach(selector => {
        ...
        inputs.forEach(input => {
            if (isVisible(input) && !input.value) { // 优先选择空的输入框
    ...
    console.log("usernameInputs", usernameInputs);
