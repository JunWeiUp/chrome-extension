...
function checkLoginSuccess() {
    // 如果没有最近的登录尝试，直接返回
    if (!lastLoginAttempt || Date.now() - lastLoginAttempt.timestamp > 5000) {
        return;
    }
    console.log("lastLoginAttempt", lastLoginAttempt);
    ...
    if (successIndicators.some(indicator => indicator)) {
        showSavePrompt(lastLoginAttempt.username, lastLoginAttempt.password);
...
// 在background.js或popup.js中处理保存账号的消息
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.action === 'saveAccount') {
        const { url, username, password } = request.data;
        
        // 获取现有账号
        chrome.storage.local.get([url], function(result) {
            const accounts = result[url] || [];
            ...
            // 保存更新后的账号列表
            chrome.storage.local.set({ [url]: accounts }, function() {
                console.log('账号已保存');
            });
        });
    }
